...
class AnalysisRunner:
    ...
    def run_signal_model_analysis(self, use_reco_mass: bool = False, fit_models: bool = False, max_workers: int = 4):
        """Run signal model analysis"""
        print("=== Running Signal Model Analysis ===")
        self.analyzer.build_signal_models(use_reco_mass, fit_models=fit_models, max_workers=max_workers)
    ...
    def run_full_analysis(self, args, analysis_max_workers: int = 4, plot_max_workers: int = 1):
        ...
        # Response function analysis is now part of the simultaneous fit
        ...
        if args.signal_model or (args.full and not args.no_signal_model):
            self.run_signal_model_analysis(args.use_reco_mass, fit_models=True, max_workers=analysis_max_workers)
...
def main():
    ...
    # Default behavior: if not provided, no nuisances are applied.
    shape_nuisance_specs = args.nuisanced_vars if args.nuisanced_vars is not None else []
