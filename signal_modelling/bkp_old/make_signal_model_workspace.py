...
def make_response_function_workspace(samples, wsfile):
    ...
    for name, sample in samples.items():
        ...
        for var in observables + vars + normalizations:
            w.factory(f"""{var}_{name}[{','.join([str(v) for v in sample[f'{var}_range']])}]""") #horrid but compact syntax

        # create and fill dataset
        data = ROOT.RooDataSet(f"response_data_{name}", f"response_data_{name}", ROOT.RooArgSet(w.var(f"reduced_mass_{name}")))
        for i in range(t.GetEntries()):
            t.GetEntry(i)
            for val in t.SelectedDiEle_fitted_mass:
                gen_mass = t.GenZd_invMass
                w.var(f"reduced_mass_{name}").setVal(val/gen_mass - 1)
                # w.var(f"reduced_mass_{name}").setVal(val/sample["nominal_mass"] - 1)
                data.add(ROOT.RooArgSet(w.var(f"reduced_mass_{name}")))
...
def make_signal_model(samples, wsfile, parametrized_vars, isParametrized = False, fit = False):
    ...
    for name, sample in samples.items():
        ...
        for var in vars_BW + vars_dCB:
            if var in parametrized_vars and isParametrized: # vars to be parametrized
                ...
                w.factory(f"""{var}_{tag}_{name}[{','.join([str(v) for v in sample[f'{var}_range']])}]""") #horrid but compact syntax
                if var in const_vars:
                    obj = w.obj(f'{var}_{tag}_{name}')
                    obj.setVal(w.obj(f'{var}_const').getVal())
                    obj.setError(w.obj(f'{var}_const').getError())
                ...
                if var in vars_BW:
                    ...
                    # val = ref_var
                    # Use fitted values
                    ref_var = w.var(f"{var}_GEN_fit_{name}").getVal()
                    w.var(f"{var}_{tag}_{name}").setVal(ref_var)
                    w.var(f"{var}_{tag}_{name}").setConstant(True)
                    val = sample["nominal_mass"] * w.obj(f"{var}_fit_par1").getVal() + w.obj(f"{var}_fit_par0").getVal()
                    # if var == "width_BW":
                    #     val = 10e-10
                    w.obj(f"{var}_{tag}_{name}").setVal(val)
                    w.var(f"{var}_{tag}_{name}").setConstant(True)
                    print(f"Setting {var}_{tag}_{name} to {val}")
                    # formula = "@0 + @1 * @2"
                    # obj = ROOT.RooFormulaVar(
                    #     f"{var}_{tag}_{name}",
                    #     f"{var}_{tag}_{name}",
        ...
        # print all variables
        for var in vars_BW + vars_dCB:
            print(f"{var}_{tag}_{name}: {w.obj(f'{var}_{tag}_{name}').getVal()}")

        ### DATASET
        # first check if data already in workspace
        data = w.data(f"data_{name}")
        if not data:
        ...
        var_string_dCB = f"mass_{name}, {','.join([f'{var}_{tag}_{name}' for var in vars_dCB])}"
        w.factory(f"CrystalBall::crystalBall_{tag}_{name}({var_string_dCB})")
        ...
        var_list_BW = [w.obj(f"{var}_{tag}_{name}") for var in vars_BW]
        ...
        relBW = ROOT.RooGenericPdf(f"relBW_{tag}_{name}", relBW_formula, var_list_BW )        
        w.Import(relBW)
        ...
        w.factory(f"FFTConvPdf::model_{tag}_{name}(mass_{name}, crystalBall_{tag}_{name}, relBW_{tag}_{name})")
        ...
        if isParametrized and fit:
            print(f"Post-parametrization fit parameters for {name}:")
            for var in vars_BW + vars_dCB:
                obj = w.obj(f'{var}_{tag}_{name}')
                val = obj.getVal()
...
def build_signal_model_for_mass(samples, wsfile, parametrized_vars, mass):
    ### WORKSPACE 
    f = ROOT.TFile.Open(wsfile)
    w = f.Get("w")
    ...
    for var in vars_BW + vars_dCB:
        if var in parametrized_vars: # vars to be parametrized
            ...
            obj = ROOT.RooFormulaVar(
                ...
                f"{var}_{tag}_{name}",
                formula,
                ROOT.RooArgList([w.obj(f"{var}_fit_par0"), w.obj(f"{var}_fit_par1"), mass])
                )
            w.Import(obj)
        else:
            w.factory(f"{var}_{tag}_{name}[0, -100, 100]")
            if var in const_vars:
                obj = w.obj(f'{var}_{tag}_{name}')
                obj.setVal(w.obj(f'{var}_const').getVal())
                obj.setError(w.obj(f'{var}_const').getError())
    ...
    var_string_dCB = f"mass_{tag}, {','.join([f'{var}_{tag}_{name}' for var in vars_dCB])}"
    w.factory(f"CrystalBall::crystalBall_{tag}_{name}({var_string_dCB})")
    ...
    w.factory(f"FFTConvPdf::model_{tag}_{name}(mass_{tag}, crystalBall_{tag}_{name}, relBW_{tag}_{name})")
    model = w.pdf(f"model_{tag}_{name}")
...
def test_BW_GEN(samples, wsfile, parametrized_vars, fit = False):
    ...
    for name, sample in samples.items():
        ...
        for var in vars_BW:
            ...
            if var in vars_BW:
                ...
                ref_var = "nominal_mass" if var == "mean_BW" else "nominal_width"
                w.var(f"{var}_{tag}_{name}").setVal(sample[ref_var])
                if not fit:
                    w.var(f"{var}_{tag}_{name}").setConstant(True)
        ...
        if not data:
            ...
            data = ROOT.RooDataSet(f"data_GEN_{name}", f"data_GEN_{name}", ROOT.RooArgSet(w.var(f"mass_GEN_{name}")))
            for i in range(t.GetEntries()):
                t.GetEntry(i)
            ...
            print(f"GEN data for {name} reloaded")
