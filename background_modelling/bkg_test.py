...
if not args.floating_signal:
    ...
    for name, model in {"jpsi" : jpsi_model, "psi2s" : psi2s_model}.items():
        ...
        # nominal_mass = mparams[f"mean_BW_test_{model.GetName().split('_')[-1]}"].getValV()
...
if args.fit_jpsi_prompt:
    ...
    print("Fitted jpsi and psi2s models to prompt data")
    # print fitted parameters
    for param in jpsi_plus_psi2s.getParameters(prompt_data):
        print(f"param: {param.GetName()}, value: {param.getValV():.5f}, error: {param.getError():.5f}")
    ...
    print(f"chi2 = {chi2_val} (n. free params = {npar})")
    ...
    legend_prompt.AddEntry(frame_prompt.findObject("data"), "Data", "p")
...
if fit_range == "unblinded" and args.fit_jpsi_first:
    print("Fitting jpsi model first")
    ...
    print("nsig1 fitted = ", w.var("nsig1").getValV(), "with fraction = ", sig_frac, " in jpsi region, ", sig_frac_unblinded, " in unblinded region")
    w.var("nsig1").setVal(w.var("nsig1").getValV() / sig_frac * sig_frac_unblinded)
    w.var("nsig1").setConstant(True)
    print(f"nsig1 = ", w.var("nsig1").getValV())
...
if fit_range == "unblinded":
    sb_fitres = sb_model.fitTo(data, ROOT.RooFit.Range("unblinded"), ROOT.RooFit.Save(), ROOT.RooFit.NumCPU(8))

# Import SB model to workspace as full_bkg_model
full_bkg_model = ROOT.RooAddPdf("full_bkg_model", "full_bkg_model",
                        ROOT.RooArgList(jpsi_model, psi2s_model, chosen_bkg),
                        ROOT.RooArgList([w.obj(var) for var in normalizations]))
...
if fit_range == "unblinded":
    ...
    # plot components
    sb_model.plotOn(frame, ROOT.RooFit.Components("model_test_M3p1"), ROOT.RooFit.LineColor(ROOT.kRed), ROOT.RooFit.Name("jpsi_model"), ROOT.RooFit.NormRange(fit_range))
    sb_model.plotOn(frame, ROOT.RooFit.Components("model_test_M3p7"), ROOT.RooFit.LineColor(ROOT.kBlue), ROOT.RooFit.Name("psi2s_model"), ROOT.RooFit.NormRange(fit_range))
...
canvas.SaveAs(os.path.join(outfolder, f"dataset{sample_suffix}{side_suffix}{tag}.png"))
canvas.SaveAs(os.path.join(outfolder, f"dataset{sample_suffix}{side_suffix}{tag}.pdf"))

# set log scale 
canvas.SetLogy()
canvas.SaveAs(os.path.join(outfolder, f"dataset{sample_suffix}{side_suffix}{tag}_log.png"))
canvas.SaveAs(os.path.join(outfolder, f"dataset{sample_suffix}{side_suffix}{tag}_log.pdf"))


# Save post-fit parameters for all fitted functions
with open(os.path.join(outfolder, f"post_fit_params{side_suffix}{tag}.log"), "w") as log_file:
    if fit_range == "unblinded":
        pdfs = [sb_model]
    else:
    ...
    for pdf in pdfs:
        print(f"Post-fit parameters for {pdf.GetName()}:")
        log_file.write(f"Post-fit parameters for {pdf.GetName()}:\n")
        for param in pdf.getParameters(data):
            print(f"\t{param.GetName()}: {param.getValV():.5f} +- {param.getError():.5f} (limits: {param.getMin():.3f}, {param.getMax():.3f})")
            log_file.write(f"\t{param.GetName()}: {param.getValV():.5f} +- {param.getError():.5f} (limits: {param.getMin():.3f}, {param.getMax():.3f})\n")
            if pdf.GetName() == "sb_model":
                # if parameter is not constant:
                if param.GetName().split("_")[0] in ["mean", "sigma", "alphaL", "nL", "alphaR", "nR"]:
                    print(f"\t\tPrefit = {prefit_param_values[param.GetName()]:.5f} (rel. variation = {(param.getValV() - prefit_param_values[param.GetName()]) / param.getValV() * 100:.2f}%)")
                    log_file.write(f"\t\tPrefit = {prefit_param_values[param.GetName()]:.5f} (rel. variation = {(param.getValV() - prefit_param_values[param.GetName()]) / param.getValV() * 100:.2f}%)\n")
    ...
    if fit_range == "unblinded":
        for pdf in [jpsi_model, psi2s_model, chosen_bkg]:
            ...
            print(f"Integral of {pdf.GetName()} in unblinded range: {integral.getVal():.5f}")
            log_file.write(f"Integral of {pdf.GetName()} in unblinded range: {integral.getVal():.5f}\n")