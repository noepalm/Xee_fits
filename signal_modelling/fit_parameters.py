...
def fit_parameters(samples, categories, wsfile, vars, parametrized_vars, gen = False):
    ...
    for category_label, category in categories.items():
        ...
        for name, sample in samples.items():
            for var in vars:
                input_var = f"{var}_GEN_fit_{name}{cat_name}" if gen else f"response_{var}_{name}{cat_name}"
        ...
        fits = {var : ROOT.TF1(f"fit{tag}_{var}{cat_name}", "[0] + [1]*x", 0, 12) for var in parametrized_vars}
        ...
        consts = {var : ROOT.RooRealVar(f"{var}{cat_name}{tag}_const", f"{var}{cat_name}{tag}_const", -1) for var in const_vars}
        ...
        for i in range(2):
            for var, fit in fits.items():
                obj = ROOT.RooRealVar(f"{var}{cat_name}_fit_par{i}", f"{var}{cat_name}_fit_par{i}", fit.GetParameter(i))
