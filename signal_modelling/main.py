...
common_ranges = {
    ...
    # # FIXME: good inits from tuning. Not sure why it still gets away
    # "reduced_mass_range" : [0, -0.6, 0.6],
    # "response_mean_range" : [0, -0.2, 0.2],
    # "response_sigma_range" : [0.02, 0, 1],
...
if __name__ == "__main__":
    ...
    if args.use_reco_mass:
        for name, sample in samples.items():
            for nparam, param in sample.items():
                # if mean_range in name:
                if "mean_range" in nparam:
                    sample[nparam] = [val + sample["nominal_mass"] for val in param]
                if "sigma_range" in nparam:
                    sample[nparam] = [val * sample["nominal_mass"] for val in param]

    # add tag to output
    if args.tag != "":
        wsfile = wsfile.replace(".root", f"_{args.tag}.root")
        eos_folder = eos_folder + f"_{args.tag}"
