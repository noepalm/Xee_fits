...
for mass in np.arange(0.1, 11.0, 0.1):
    ...
    var_up = w.var(f"Zd_M{mass:.1f}_expected_electronID_up")
    var_down = w.var(f"Zd_M{mass:.1f}_expected_electronID_down")
...
ax.plot(masses, up_values, label="Electron ID Up", marker='o', markersize=3)
ax.plot(masses, down_values, label="Electron ID Down", marker='o', markersize=3)
...
for ext in ["png", "pdf"]:
    plt.savefig(Path(outfolder) / f"electron_id_yield_variation.{ext}")

fig, ax = plt.subplots(figsize=(10, 10))
up_values_fraction = abs(1-np.array(up_values))
down_values_fraction = abs(1-np.array(down_values))
ax.plot(masses, up_values_fraction * 100, label="Electron ID Up", marker='o', markersize=3)
ax.plot(masses, down_values_fraction * 100, label="Electron ID Down", marker='o', markersize=3)
ax.set_xlabel("$M(Z_D)$ [GeV]")
...
for ext in ["png", "pdf"]:
    plt.savefig(Path(outfolder) / f"electron_id_yield_variation_minus1.{ext}")
