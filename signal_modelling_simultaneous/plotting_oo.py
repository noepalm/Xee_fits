...
class ParametrizationPlotter:
    ...
    def _collect_parametrization_data(self, samples: Dict[str, SampleConfig], 
        ...
        for name, sample in samples.items():
            ...
            entry_counts = self.analyzer.dataset_loader.get_dataset_entry_count(f"data_{sample.label}{category.label}_{self.analyzer.era}")
    ...
    def _plot_parameter_fit_only(self, ax, var: str, category: CategoryConfig, color: str, 
        ...
        if var in self.analyzer.parametrized_vars:
            ...
            # print("DEBUG: Retrieved fit parameters:", par0_obj, par1_obj)
            # print("DEBUG: Retrieved fit parameter errors:", par0_err_obj, par1_err_obj, par01_cov_obj)
            ...
            if par0_obj and par1_obj:
                ...
                if show_fit_params:
                    ...
                    if par0_err_obj.getVal() > 0 and par1_err_obj.getVal() > 0:
...
class ModelPlotter:
    ...
    def _plot_single_fit(self, sample: SampleConfig, category: CategoryConfig, 
        ...
        if plot_type == "response":
            ...
            obs_var = self.workspace.var(f"{obs_name}")
            ...
            tag = "_GEN" if gen else ""
            model_tag = "_GEN" if gen else "_test" #"_param" before
        ...
        # Create frame
        print("DEBUG: Using observable ", obs_var)
        ...
        if plot_fit:
            ...
            print("DEBUG: retreiving model with name:", model_name)
            # print("DEBUG: available models in workspace:", [obj.GetName() for obj in self.workspace.allPdfs()])
            ...
            if model:
                ...
                model.plotOn(frame, ROOT.RooFit.Name(model_legend_name), 
                            ROOT.RooFit.LineWidth(2), ROOT.RooFit.LineColor(color), ROOT.RooFit.NormRange(sample.label))
