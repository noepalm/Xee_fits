...
eos_folder = "/eos/home-n/npalmeri/www/DiElectron/signal_model/tests/forPresentation_test"
...
def plot_response_fit(samples, wsfile, plot_fit = True):
    ...
    for name, sample in samples.items():
        ...
        frame = w.var(f"reduced_mass_{name}").frame(
            ROOT.RooFit.Title(f"Reduced mass distribution of {name} sample")
        )
        data.plotOn(frame)
        ...
        frame.GetXaxis().SetTitle(r"(m(ee) - mX)/mX [GeV]")
...
def compare_zd_jpsi_shape(samples, wsfile, plot_fit = True):
