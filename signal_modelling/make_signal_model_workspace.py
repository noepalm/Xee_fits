...
def make_response_function_workspace(samples, wsfile, use_reco_mass = False):
    ...
    for name, sample in samples.items():
        ...
        w.factory(f"""CrystalBall::response_function_{name}({','.join([f"{var}_{name}" for var in observables + vars])})""")
        # w.factory(f"""ExtendPdf::response_function_{name}(crystalBall_response_{name}, response_nsgn_{name})""")
...
def make_signal_model(samples, wsfile, parametrized_vars, isParametrized = False, fit = False, use_reco_mass = False):
    ...
    for name, sample in samples.items():
        ...
        if use_reco_mass:
            model = w.pdf(f"crystalBall_{tag}_{name}").Clone(f"model_{tag}_{name}")
        else:
...
def build_signal_model_for_mass(samples, wsfile, parametrized_vars, mass, use_reco_mass = False):
    ### WORKSPACE 
    f = ROOT.TFile.Open(wsfile)
    w = f.Get("w")
...
def test_BW_GEN(samples, wsfile, parametrized_vars, fit = False):
    ...
    for name, sample in samples.items():
        ...
        relBW = ROOT.RooGenericPdf(f"relBW_{tag}_{name}", relBW_formula, var_list_BW)
        w.Import(relBW)
        model = relBW.Clone(f"model_{tag}_{name}")
