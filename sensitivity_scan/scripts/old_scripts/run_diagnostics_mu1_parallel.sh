...
process_dir() {
    ...
    if [ -f Xee_ee_0_2023.txt ]; then
        ...
        #         --skipBOnlyFit \
        #         -n .injectedSignal \
        #         -v 1 &> fitDiagnostics_injectedSignal.log;

        # inject signal into the workspace
        python3 $BASEDIR/scripts/inject_signal.py -i Xee_ee_0_2023.root -m $mass -o Xee_ee_0_2023_injected_mu1.root

        combine -M FitDiagnostics Xee_ee_0_2023_injected_mu1.root \
            --saveNormalizations \
            --saveShapes \
            --setParameterRanges mass=2,4.2 \
            --keepFailures \
            --skipBOnlyFit \
            -n .injectedSignal_mu1 \
            -v 1 &> fitDiagnostics_injectedSignal_mu1.log;
            # --preFitValue 1 \
        ...
        # copy output
        # cp fitDiagnostics_injectedSignal.log fitDiagnostics.injectedSignal.root higgsCombine.injectedSignal_mu1.FitDiagnostics.mH120.123456.root $OUTFOLDER/M$mass/
        cp fitDiagnostics_injectedSignal.log fitDiagnostics.injectedSignal.root higgsCombine.injectedSignal_mu1.FitDiagnostics.mH120.root $OUTFOLDER/M$mass/
        cp combine_logger.out $OUTFOLDER/M$mass/combine_logger_fitDiagnostics_injectedSignal_mu1.out

        python3 $BASEDIR/scripts/draw_mu1_fit.py -i Xee_ee_0_2023_injected_mu1.root -d Xee_ee_0_2023.root -f fitDiagnostics.injectedSignal_mu1.root -o $OUTFOLDER -m $mass 
...
python3 $BASEDIR/scripts/plot_diagnostics_result.py -o $OUTFOLDER -m 1